...
def encode_obs(observation):  # For DP
    ...
    obs = dict(
        head_cam=head_cam, front_cam=front_cam, left_cam=left_cam, right_cam=right_cam
    )
...
def eval(TASK_ENV, model, observation):
    ...
    # ======== Get Action ========
    if len(model.runner.obs) == 0:  # TODO check if this is necessary
        model.update_obs(obs)
    actions = model.get_action()  # Get action from model type & shape?
...
def reset_model(model):
    model.runner.reset_obs()  # TODO check if this is necessary
